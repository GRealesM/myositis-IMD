...
proj.table <- fread("../data/Projection_IMD_basis_20211105-v1.tsv")
QC.table <- fread("../data/QC_IMD_basis_20211105-v1.tsv")
metadata <- fread("../data/Metadata_20211105-v1.tsv")
...
# Remove FinnGen R2 and R3, so we keep R5 only
...
QC.filt <- QC.table[nSNP >= max(nSNP) * 0.8 ][!grepl("FinnGenR[23]", Trait, perl = TRUE)][!Trait %in% basis.datasets][First_Author == "Rothwell"  | !grepl("ImmunoChip", Chip, ignore.case = TRUE)]
...
independent <- c("CD_Liu_26192919_1", "K11_PSC_COLITIS_FinnGen_FinnGenR5_1", "UC_Liu_26192919_1", "SLE_Julia_29848360_1", "20002_1456_PanUKBB_PanUKBBR1_1", "MS_IMSGC_31604244_1", "20002_1111_Neale_UKBB_1", "20002_1661_PanUKBB_PanUKBBR1_1", "M13_RHEUMA_FinnGen_FinnGenR5_1", "T1D_Chiou_34012112_1")
guide <- c("CD", "PSC", "UC", "SLE", "CEL", "MS", "Asthma", "VIT", "RA", "T1D")
...
save_pheatmap_png(bp, filename = "../figures/202111_Nov21/Basis_heatmap_r.png", width = 1500, height = 1500)
save_pheatmap_svg(bp, filename = "../figures/202111_Nov21/Basis_heatmap_r.svg", width = 8, height = 8)
...
```{r eval = FALSE}
save_pheatmap_png(my, filename = "../figures/202111_Nov21/Myo_heatmap_r.png", width = 1500, height = 1500)
save_pheatmap_svg(my, filename = "../figures/202111_Nov21/Myo_heatmap_r.svg", width = 8, height = 8)
...
To see our datasets in context, we'll make ladder plots for the crucial PCs (1, 12, and 13)
...
make.labels <- function(x){
  x[,Label:=Trait_long] %>% 
    ...
    .[, Label:=paste0(str_trunc(paste0(Label, " (", Population,")"), width = 50), " / ", First_Author)] %>% 
    .[, Label:=gsub("Neale", "UKBB", Label)]

}

# Load clustering files, just to extract selection
load("../data/bigpsm.RData") # To extract the labels and traits
sel <- rownames(bigpsm)
rm(bigcalls,bigpsm)

# Now we'll prepare PT.sig to merge with cluster traits
PT.sel <- PT.sig[!grepl("PanUKBB", First_Author)]                                                                     # Remove PanUKBB, as we're using Neale UKBB only
PT.sel[, shorttrait:=ifelse(First_Author == "Neale", Trait_long, Trait)][, shorttrait:=shorten_names(shorttrait)] # Create a shorttrait column and apply function

# Add Rothwell to the list
sel <- c(sel, unique(PT.sel$shorttrait[grepl("Rothwell", PT.sel$shorttrait)]))

PT.sel <- PT.sel[shorttrait %in% sel]   
# Create appropriate labels
PT.sel <- make.labels(PT.sel)
PT.sel[,shorttrait:=NULL]
...
Save the plots
```{r eval=FALSE}
ggsave("../figures/202111_Nov21/Ladder_plot_PC1.png", forest.plots[["PC1"]], units = "mm", width = 180, height = 220, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Ladder_plot_PC3.png", forest.plots[["PC3"]], units = "mm", width = 180, height = 220, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Ladder_plot_PC9.png", forest.plots[["PC9"]], units = "mm", width = 180, height = 140, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Ladder_plot_PC12.png", forest.plots[["PC12"]], units = "mm", width = 180, height = 159, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Ladder_plot_PC13.png", forest.plots[["PC13"]], units = "mm", width = 180, height = 220, dpi = 300, bg="white")
```
...
# 450 traits. Still a lot, but let's move forward with this
...
```{r eval= FALSE}
save_pheatmap_png(hmred, filename = "../figures/202111_Nov21/Redsig_heatmap_v1.png", width = 1500, height = 7000)
```
...
PT.delta <- PT.redviz[FDR.PC < 0.01 | First_Author %in% c("Miller", "Rothwell")] # Significant projections at FDR 1% or Myositis (Miller or Rothwell).
PT.delta <- rbindlist(list(PT.basis, PT.delta),fill = TRUE) # Merge everything
PT.delta[, Var.Delta:=as.numeric(Var.Delta)]
PT.delta[Var.Delta != 0, ci:=sqrt(Var.Delta) * 1.96][is.na(ci),ci:=0] 
PCs <- unique(PT.delta$PC)
...
Save the plots
```{r eval=FALSE}
ggsave("../figures/202111_Nov21/Redviz_Ladder_plot_PC1.png", forest.plots[["PC1"]], units = "mm", width = 250, height = 700, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Redviz_Ladder_plot_PC2.png", forest.plots[["PC2"]], units = "mm", width = 250, height = 400, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Redviz_Ladder_plot_PC3.png", forest.plots[["PC3"]], units = "mm", width = 250, height = 600, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Redviz_Ladder_plot_PC9.png", forest.plots[["PC9"]], units = "mm", width = 250, height = 300, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Redviz_Ladder_plot_PC12.png", forest.plots[["PC12"]], units = "mm", width = 250, height = 450, dpi = 300, bg="white")
ggsave("../figures/202111_Nov21/Redviz_Ladder_plot_PC13.png", forest.plots[["PC13"]], units = "mm", width = 250, height = 700, dpi = 300, bg="white")
```
