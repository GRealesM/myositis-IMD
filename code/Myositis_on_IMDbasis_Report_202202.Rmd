...
QC.table <- merge.data.table(QC.table, metadata) 
...
independent <- c("20002_1111_Neale_UKBB_1",  "M13_RHEUMA_FinnGen_FinnGenR5_1", "20002_1456_PanUKBB_PanUKBBR1_1", "MS_IMSGC_31604244_1", "ph571.6_PanUKBB_PanUKBBR1_1", "SLE_Julia_29848360_1", "20002_1661_PanUKBB_PanUKBBR1_1", "T1D_Chiou_34012112_1", "CD_Liu_26192919_1", "UC_Liu_26192919_1", "K11_PSC_COLITIS_FinnGen_FinnGenR5_1")
guide <- c("Asthma","RA", "CEL", "MS", "PBC", "SLE", "VIT", "T1D", "CD", "UC", "PSC")
...
IMDorder <- c("Asthma","RA", "CEL", "IgA_Neph", "MS", "PBC", "SLE", "LADA", "VIT", "T1D", "CD", "UC", "PSC") # Following Burren et al. grouping
...
M.sig <- M.sig[PCorder, IMDorder]
M.sig.stars <- M.sig.stars[PCorder, IMDorder]
range <- max(abs(M.sig))
bp <- pheatmap(M.sig, breaks = seq(-range, range, length.out =100), cluster_cols = FALSE, cluster_rows = FALSE, display_numbers = M.sig.stars, treeheight_col = 15, fontsize_row = 12, fontsize_col = 12,cellwidth = 15, cellheight = 15, fontsize_number = 14,  color = rev(hmcol), legend = T)
bp
```
...
save_pheatmap_png(bp, filename = "../figures/202202_Feb22/Basis_heatmap_r.png", width = 1500, height = 1500)
save_pheatmap_svg(bp, filename = "../figures/202202_Feb22/Basis_heatmap_r.svg", width = 8, height = 8)
...
myo.order <- c("Myositis / Miller", "Myositis / Rothwell", "Polymyositis / Miller", "Polymyositis / Rothwell", "Inclusion Body Myositis / Rothwell", "Jo1+ Myositis / Rothwell", "Dermatomyositis / Miller", "Dermatomyositis / Rothwell", "Juvenile Dermatomyositis / Miller","Juvenile Dermatomyositis / Rothwell")
...
M.myo <- M.myo[PCorder, myo.order]
M.myo.stars <- M.myo.stars[PCorder, myo.order]
range <- max(abs(M.myo))
my <- pheatmap(M.myo, breaks = seq(-range, range, length.out =100), cluster_cols = FALSE, cluster_rows = FALSE, display_numbers = M.myo.stars, treeheight_col = 15, fontsize_row = 12, fontsize_col = 12,cellwidth = 15, cellheight = 15, fontsize_number = 14,  color = rev(hmcol), legend = T)
my
```
...
```{r eval = FALSE}
save_pheatmap_png(my, filename = "../figures/202202_Feb22/Myo_heatmap_r.png", width = 1500, height = 1500)
save_pheatmap_svg(my, filename = "../figures/202202_Feb22/Myo_heatmap_r.svg", width = 8, height = 8)
...
We manually selected a number of immune conditions for clustering together with Miller's myositis datasets.
...
PT.forst2 <- copy(PT.delta) # For tables later
...
Save the plots
```{r eval=FALSE}
ggsave("../figures/202202_Feb22/Ladder_plot_PC1.png", forest.plots[["PC1"]], units = "mm", width = 180, height = 220, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Ladder_plot_PC2.png", forest.plots[["PC2"]], units = "mm", width = 180, height = 180, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Ladder_plot_PC3.png", forest.plots[["PC3"]], units = "mm", width = 180, height = 220, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Ladder_plot_PC8.png", forest.plots[["PC8"]], units = "mm", width = 180, height = 160, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Ladder_plot_PC9.png", forest.plots[["PC9"]], units = "mm", width = 180, height = 140, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Ladder_plot_PC12.png", forest.plots[["PC12"]], units = "mm", width = 180, height = 180, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Ladder_plot_PC13.png", forest.plots[["PC13"]], units = "mm", width = 180, height = 260, dpi = 300, bg="white")
```
...
```{r eval= FALSE}
save_pheatmap_png(hmred, filename = "../figures/202202_Feb22/Redvizsig_heatmap_v1.png", width = 1500, height = 9000)
```
...
Save the plots
```{r eval=FALSE}
ggsave("../figures/202202_Feb22/Redviz_Ladder_plot_PC2.png", forest.plots[["PC2"]], units = "mm", width = 300, height = 500, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Redviz_Ladder_plot_PC3.png", forest.plots[["PC3"]], units = "mm", width = 300, height = 900, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Redviz_Ladder_plot_PC8.png", forest.plots[["PC8"]], units = "mm", width = 300, height = 450, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Redviz_Ladder_plot_PC1.png", forest.plots[["PC1"]], units = "mm", width = 300, height = 900, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Redviz_Ladder_plot_PC9.png", forest.plots[["PC9"]], units = "mm", width = 300, height = 300, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Redviz_Ladder_plot_PC12.png", forest.plots[["PC12"]], units = "mm", width = 300, height = 550, dpi = 300, bg="white")
ggsave("../figures/202202_Feb22/Redviz_Ladder_plot_PC13.png", forest.plots[["PC13"]], units = "mm", width = 300, height = 1000, dpi = 300, bg="white")
```
...
We'll now save some of these tables to be used as Supplementary Tables in the paper.

```{r eval=FALSE}
# Info on *all* GWAS projected
st1 <- copy(QC.table)
st1[, c("SNP_number"):=NULL][, sig_overall:=ifelse(Trait %in% QC.sig$Trait, "Yes", ifelse(Trait %in% QC.filt$Trait, "No", "Filtered"))]
fwrite(st1, "../tables/SupplTable_GWAS_info.tsv", sep = "\t")

# Info on *all* projections
st2 <- copy(proj.table)
st2 <- merge(st2, PT.filt[, .(Trait, PC, Trait_class, FDR.PC, stars)], by=c("Trait", "PC"), all.x = TRUE)
st2[ Trait %in% st1[ sig_overall == "No"]$Trait, Projection_type:="Not significant overall"]
st2[ Trait %in% st1[ sig_overall == "Yes"]$Trait, Projection_type:="Significant overall"]
st2[ Trait %in% st1[ sig_overall == "Filtered"]$Trait, Projection_type:="Filtered"]
st2[ is.na(Projection_type), Projection_type:= "Basis"]
st2 <- merge(st2, unique(PT.forst2[, .(Trait, Label)]), by="Trait", all.x = TRUE)

fwrite(st2, "../tables/SupplTable_Projections_complete.tsv", sep = "\t")

# Info on Myositis datasets (main table)
t3 <- fread(input = "DMY_Miller_26291516_1-hg38.tsv.gz 9372705
DMY_Rothwell_up_1-hg38.tsv.gz 1648116
IBM_Rothwell_up_1-hg38.tsv.gz 1648116
IIM_Rothwell_up_1-hg38.tsv.gz 1648116
JDM_Miller_26291516_1-hg38.tsv.gz 9373293
JDM_Rothwell_up_1-hg38.tsv.gz 1648116
JO1M_Rothwell_up_1-hg38.tsv.gz 1648116
MYO_Miller_26291516_1-hg38.tsv.gz 9373243
PM_Miller_26291516_1-hg38.tsv.gz 9372754
PM_Rothwell_up_1-hg38.tsv.gz 1648116
", header=FALSE)
names(t3) <- c("Trait", "Total_SNP")
t3$Trait <- gsub("-hg38.tsv.gz", "", t3$Trait)
t3 <- merge(t3, QC.table[, .(Trait, N0, N1, nSNP)], by="Trait")
t3 <- merge(t3, unique(PT.forst2[, .(Trait, Label)]), by="Trait")
setcolorder(t3, c("Trait", "Label", "Total_SNP", "nSNP", "N0", "N1"))
fwrite(t3, "../tables/MainTable_Myositis_info.tsv", sep = "\t")
