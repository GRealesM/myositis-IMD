...
We want to learn more about myositis and its subtypes in the light of other IMD. For that we'll take a closer look at myositis datasets from two studies (Miller et al., 2015 and Rothwell et al. 2022) projected onto our IMD-trained features together with projections from our curated collection of IMD summary statistics from multiple sources. We'll cluster these projections, compute genetic similarity, and follow-up on key SNPs underlying the observed patterns, using colocalisation. 
...
DPMUnc finds 10 clusters across 7 features. All myositis datasets fall on the same cluster (1) except for FinnGen's myositis (cluster 3). This is likely due to the different composition of the FinnGen myositis set, comprising a majority of unclassified myositis cases, while Miller and Rothwell general myositis feature meta-analyses of classified myositis subtypes.
...
Test plot: an attempt to "desconstruct" the biplots

```{r echo=FALSE, fig.height= 8, fig.width=8}
p7.1 <- p7[DPMUnc.cl ==1]
p7.1[grepl("myositis", Label, ignore.case = TRUE), group:="myositis"][is.na(group), group:="other_IMD"][, group:=as.factor(group)]
p7.1 <- p7.1[, .(Label, Delta_PC1,Delta_PC2, Delta_PC3, Delta_PC8, Delta_PC9, Delta_PC12, Delta_PC13, group)]

# tpt <- ggparcoord(p7.1, columns = c(2:8), groupColumn = "Label",
#          showPoints = TRUE,
#          title = "Parallel Coordinate Plot for Cluster 1",
#          alphaLines = 0.3
#     ) +
#   geom_text_repel(data = . %>% filter(variable == "Delta_PC1"),
#                   aes(x = variable, y = value, label = Label),
#                   xlim = c(NA, 1), hjust=1) + # limit repel region to the left of the 1st column
#   theme_minimal()+
#   theme(legend.position = "none")


tpt <- ggparcoord(p7.1, columns = c(2:8), groupColumn = "group",
         showPoints = TRUE,
         title = "Parallel Coordinate Plot for Cluster 1",
         alphaLines = 0.3
    ) +
...
* Autoimmune hyperthyroidism (hyperthy)
* Hypothyroidism, congenital or acquired (hypothy)


![Manhattan plot of driver SNPs FDR across IMDs](../figures/manhattan_24IMD.png)
...
We computed the pairwise FDR for all myositis-IMD pairs and filtered out SNPs in close proximity (< 1Mb apart), resulting in 61 myositis-IMD significant pairs comprising 15 driver SNPs and 11 clusters or independent, non-overlapping regions. None of the 15 SNPs with shared associations are genomewide significant (P < 5e-8) in their individual GWAS (see pdriver.myos in the table below).


```{r echo=FALSE}
options(knitr.kable.NA = '')
coloc <- fread("../tables/coloc_results.tsv")
...
To explore a wider range of driver SNPs across myositis-IMD pairs, we relaxed the FDR threshold to 0.5 (ie. more likely to be significant for both diseases than not), and we obtained 77 driver SNPs and 477 myositis-IMD pairs. We applied coloc to each myositis-IMD pair regions surrounding each driver SNP associated to the pair, using a 1Mb window around and assuming a single causal variant. 


This resulted in 15 driver SNPs (xx independent regions) with $P(H4) >0.5$ (ie. highest probability for a shared variant for both IMD in the region).

As mentioned earlier, the driver SNP might not be the causal one, but be in LD with the causal variant instead. Thus, we also report SNP with the highest posterior probability of being the shared causal variant in each region. We'll look at each region in turn, by looking at both coloc tables and Manhattan plots. 
...
```{r echo=FALSE}
coloc[ pid == "1:113834946" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```
...
None of these SNPs are genomewide significant for either Myositis (Rothwell) or Polymyositis (Rothwell or Miller), so they can be considered novel associatons.

In the plot below, we show the region for all IMD involved. Note that some of them did have pairwise_fdr < 0.5 but not H4 > 0.5, so we don't discuss them.
...
```{r echo=FALSE}
coloc[ pid == "2:100215693" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```


![2:191071078 region Manhattan plot. The driver SNP is signalled by a red line.](../figures/coloc_chr2_1.png)

Coloc finds a shared association in this 2:100215693 region for Myositis (IIM, Rothwell) and JIA, which 2:100143015 as the SNP with the highest posterior probability to be the shared SNP (albeit slightly below the conventional H4 > 0.8 threshold for high-confidence association). [2:100143015](https://genetics.opentargets.org/Variant/2_100143015_G_A/associations) is an usptream variant mapped to AFF3, a tissue-restricted nuclear transcriptional activator preferentially expressed in lymphoid tissue. This variant has been associated with multiple blood measurements, such as mean platelet volume and blood cell count. This is a novel association for Myositis, not significant in the individual myositis (Rothwell) GWAS.
...
```{r echo=FALSE}
coloc[ pid == "2:191071078" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```


![2:191071078 region Manhattan plot. The driver SNP is signalled by a red line.](../figures/coloc_chr2_2.png)

We find two H4 > 0.5 associations in this region between Myositis (Rothwell), hyperthyroidism and systemic sclerosis. While H4 is borderline (H4 = 0.505) for hyperthyroidism, it is very high (H4 > 0.99) for systemic sclerosis. In the latter case, the SNP with the highest posterior probability to be the shared SNP is [2:191038032](https://genetics.opentargets.org/variant/2_191038032_G_A), an intron variant of **STAT1** not genomewide significant for myositis (Rothwell) but associated with other IMD, such as SLE and RA.
...
```{r echo=FALSE}
coloc[ pid == "3:28029953" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```
...
```{r echo=FALSE}
coloc[ pid %in% c("7:128933913", "7:128954129", "7:128977412") & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```
...
We find associations among four myositis datasets (Myositis [Miller and Rothwell], dermatomyositis [Miller], and Jo1+ myositis [Rothwell]) and four IMD (Sjögren's, Systemic sclerosis, MPO+ AAV). These have six associated candidate SNPs with high probability, that we'll discuss in turn:
...
* [7:128977412](https://genetics.opentargets.org/variant/7_128977412_A_G) (Myositis [Rothwell] and PAPS) -rs12531711, an intron variant of **IRF5**, previously linked to SLE, and PBC.
...
None of these candidate SNPs are genomewide significant for any of the myositis datasets in the pairs.
...
```{r echo=FALSE}
coloc[ pid =="8:11491677" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```


We find multiple associations in this region, including Myositis, Juvenile dermatomyositis (Rothwell) and dermatomyositis (Miller) with Systemic sclerosis, PAPS, and Sjögren's syndrome.
There are three candidate SNPs that we'll discuss in turn:
...
* [8:11480861](https://genetics.opentargets.org/Variant/8_11480861_A_C/associations) (JDM [Rothwell] and Myositis [Miller + Rothwell] with PAPS) - rs2618444, a regulatory region variant mapped to **FAM167A**. 
...
Again, all these candidate SNPs are novel findings for myositis.
...
```{r echo=FALSE}
coloc[ pid %in% c("11:64329761",  "11:64362250") & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```
...
* [11:64369120](https://genetics.opentargets.org/Variant/11_64369120_G_T/associations) - rs1662188, an intron variant of **RPS6KA4**.
...
```{r echo=FALSE}
coloc[ pid == "12:112468611" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```


In chromosome 12, we find two strong (H4 > 0.8) associations between myositis (Rothwell) with hypothyroidism (Candidate SNP 12:111535554) and JIA (12:111472415).
...
These are also novel hits for myositis.
...
```{r echo=FALSE}
coloc[ pid == "17:39913696" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H1,H2, H3, H4, bestsnp, bestsnp.pp, pbest.myos)] %>% kable(digits = 32) %>% kable_styling
```


In this region, we find two strong (H4 > 0.8) associations between myositis (Rothwell) with hypothyroidism (Candidate SNP 12:111535554) and JIA (12:111472415).
