...
make.custom.forest.plots <- function(ps, bs, feature, palette = tfp, fsize = NULL, threshold_basis=0, threshold_gen = 0, remove_sum = FALSE, remove_perc = FALSE, remove_side = "none"){
      pdelta <- ps[stars == "●"][Trait_class == "IMD"] # Remove all blood cells save for key ones
      pdelta <- rbind(pdelta, bs, fill = TRUE)
      pdelta[Var.Delta != 0, ci:=sqrt(Var.Delta) * 1.96][is.na(ci),ci:=0][is.na(Label), Label:=Trait]
      pdelta[ ci == 0, Trait_class:= "Basis"]
      pdelta[ Trait_class == "Basis" & Label == "Asthma", Label:="AST"] # Rename basis Asthma to avoid confusion with external asthma
      pdelta[ First_Author %in% c("Miller", "Rothwell"), Trait_class:="Myo"] # Make myositis a different colour
      pdelta[, Label:=stringr::str_trunc(Label, 50, ellipsis = " [...]")]
...
tbh <- bh.ph$tree_row
tbhc <- cutree(tbh, k=15) # manually adjusted to capture visually-selected IMD
bhatta.sel <- names(tbhc[tbhc ==9])
bhatta.sel
...
pcbiplot <-  ggplot(p7, aes(x = Delta_PC1, y = Delta_PC2, colour = DPMUnc.cl, label=Label))+
    ...
    geom_label_repel(size = 4, seed = 15, force = 20, max.overlaps = 20, data = p7[grepl("myositis|IIM", Label, ignore.case = TRUE) | Delta_PC2 > 0.075], show.legend = FALSE) +
...
pcbiplot_112 <-  ggplot(p7, aes(x = Delta_PC1, y = Delta_PC12, colour = DPMUnc.cl, label=Label))+
    ...
    scale_colour_manual(values = colp)+
    scale_fill_manual(values = colp)+
    geom_label_repel(size = 4, seed = 15, force = 20, max.overlaps = 20, data = p7[grepl("myositis|IIM", Label, ignore.case = TRUE)], show.legend = FALSE) +
  #| grepl("myastenia", Label, ignore.case = T)
    xlab("Delta PC1")+
    ylab("Delta PC12")+
    theme_minimal()+
...
pcbiplot_113 <-  ggplot(p7, aes(x = Delta_PC1, y = Delta_PC13, colour = DPMUnc.cl, label=Label))+
    ...
    scale_colour_manual(values = colp)+
    scale_fill_manual(values = colp)+
    geom_label_repel(size = 4, seed = 15, force = 20, max.overlaps = 20, data = p7[grepl("myositis|IIM", Label, ignore.case = TRUE) | Delta_PC13 > 0.07], show.legend = FALSE) +
  #| grepl("myastenia", Label, ignore.case = T)
    xlab("Delta PC1")+
    ylab("Delta PC13")+
    theme_minimal()+
...
resdpmunc
resbhat <- data.table(Label = names(tbhc), Bhatta.cl = tbhc)
#fwrite(resbhat, "../data/bhattacharyya_clustering.tsv", sep = "\t")
rescl <- merge(resdpmunc, resbhat, by ="Label")

clsum=rescl[,.(y=.N),by=c("DPMUnc.cl","Bhatta.cl")]


# Plot Relationship between disease clusterings
sdiag <- ggplot(clsum, aes(y=y, axis1=Bhatta.cl,axis2=DPMUnc.cl)) + geom_alluvium(aes(fill=factor(DPMUnc.cl))) +
            geom_stratum(width = 1/5,  fill="white", color = "black") +
            #geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
            geom_text(stat = "stratum", aes(label = after_stat(stratum)))+
            scale_x_discrete(limits = c("Bhattacharyya", "DPMUnc"), expand = c(.05, .05)) +
            scale_fill_manual(values = palette[c(3,5,11,10,6, 14,16,17,20,30)])+
...
sdiag
#ggsave("../figures/sdiag_Bh_DP.png", sdiag, height = 9, width = 7, bg = "white")
...
# Add proper labels
ml <- data.table(mlabel = c("DM (M)", "DM (R)", "JDM (M)","JDM (R)", "Jo1+ (R)", "IIM (M)", "IIM (R)", "PM (M)", "PM (R)"),
...
fig.label = c("ITP", "IgG+ NMO", "PBC", "JIA", "CR(E)ST", "MG", "Felty", "SjS", "SSc", "GPA", "MPO+ AAV", "PR3+ AAV", "HyperThy", "HypoThy"))
...
cdis[First_Author %in% c("Miller", "Rothwell") | Label %in% bhatta.sel, Bhattacharyya:="Yes"][First_Author %in% c("Miller", "Rothwell") | Label %in% dpimd, DPMUnc:="Yes"][First_Author %in% c("Miller", "Rothwell") | !is.na(coloc.label), coloc:="Yes"][is.na(Bhattacharyya), Bhattacharyya:="No"][is.na(Bhattacharyya), Bhattacharyya:="No"][is.na(coloc), coloc:="No"]
...
cts <- coloc[H4 > 0.5, .(driver.rsid, driver.nearestgene, bestsnp.rsid, mlabel, trait.other, pairwise_fdr, H4, pbest.myos, pbest.myos.region, bestsnp.novel )][order(driver.nearestgene, bestsnp.rsid)]
cts <- merge(cts, fc[, .(coloc.label, fig.label)], by.x = "trait.other", by.y = "coloc.label")
...
sumc <- merge(sumc, fc[, .(coloc.label, fig.label)], by.x = "trait.other", by.y = "coloc.label")
sumc <- sumc[, .(dlabel, mlabel, fig.label, H4, flag)]


gp1 <-  ggplot(sumc[ grepl(pattern = "^DM|JDM \\(M\\)", mlabel) ], aes(x =  fig.label, y = dlabel, colour=flag, fill = H4)) +
              geom_tile( color = "black",
                        lwd = 0.2,
                        linetype = 1) +
              scale_fill_gradient(limits = c(0,1), na.value = "white")+
              geom_tile(aes( colour = flag),
                          lwd=1, linetype = 1, data = sumc[grepl(pattern = "^DM|JDM \\(M\\)", mlabel) & H4 > 0.25]) + # Little trick to keep tiles in place
...
gp2 <-  ggplot(sumc[ grepl(pattern = "PM|JDM \\(R\\)", mlabel) ], aes(x =  fig.label, y = dlabel, colour=flag, fill = H4)) +
              geom_tile( color = "black",
                        lwd = 0.2,
                        linetype = 1) +
              scale_fill_gradient(limits = c(0,1), na.value = "white")+
              geom_tile(aes( colour = flag),
                          lwd=1, linetype = 1, data = sumc[grepl(pattern = "PM|JDM \\(R\\)", mlabel) & H4 > 0.25]) + # Little trick to keep tiles in place
              scale_colour_manual(values=c(High="green",Med="yellow", Low ="#FFFFFF00"), guide=guide_none()) + # And make rectangles under 0.5 transparent
...
gp3 <-  ggplot(sumc[ grepl(pattern = "IIM|Jo1", mlabel) ], aes(x =  fig.label, y = dlabel, colour=flag, fill = H4)) +
              geom_tile( color = "black",
                        lwd = 0.2,
                        linetype = 1) +
              scale_fill_gradient(limits = c(0,1), na.value = "white")+
              geom_tile(aes( colour = flag),
                          lwd=1, linetype = 1, data = sumc[grepl(pattern = "IIM|Jo1", mlabel) & H4 > .5]) +
...
gp <- plot_grid(gp1, gp2, gp3, nrow = 3, rel_heights = c(0.8,0.8,1.3))
 ggsave("../figures/driverSNP_H4.png", gp, height =7 , width = 7.5, bg="white")
...
#               labs(fill = "PP")
# grplot
# #ggsave("../figures/driverSNP_H4.png", grplot, height = 3.5, width = 15, bg="white")
# 
# 
# grplot <-  ggplot(sumc, aes(x =  dlabel, y = trait.other, colour=flag, fill = H4)) +
#               geom_tile( color = "black",
#                         lwd = 0.2,
#                         linetype = 1) +
#               scale_fill_gradient(na.value = "white")+
...
#                     )+
#               facet_grid(rows = vars(mlabel), scales = "free", space = "free",switch = "x")+
#               labs(fill = "PP")
# grplot
# ggsave("../figures/driverSNP_H4.png", grplot, height =13 , width = 5, bg="white")


# grplot <-  ggplot(sumc, aes(x =  trait.other, y = mlabel, colour=flag, fill = H4)) +
#               geom_tile( color = "black",
#                         lwd = 0.2,
#                         linetype = 1) +
#               scale_fill_gradient(na.value = "white")+
...
#                     strip.text.y = element_text(angle = 0)
#                     )+
#               facet_grid(rows = vars(dlabel), scales = "free", space = "free",switch = "x")+
#               labs(fill = "PP")
# grplot
