...
We want to learn more about myositis and its subtypes in the light of IMD. For that we'll take a look at myositis projections from two studies (Miller et al., 2015 and Rothwell et al. 2022) together with our other 
...
The Mahalanobis distance is heavily right-skewed, so we'll show log(1 + mahalanobis) instead, which will help us distinguish colours: 
...
tlis.ph
ggsave("../figures/Myositis_mahalanobis_heatmap.png", tlis.ph, width = 14, height = 6.5, bg="white")
...
resdpmunc[grepl("Myositis", Label, ignore.case = TRUE)]
resdpmunc[DPMUnc.cl == 6]
```
