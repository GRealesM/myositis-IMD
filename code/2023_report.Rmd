...
Mah.jdmF <- mahal.f(i=Map, var.proj = var.proj, ref="Juvenile Dermatomyositis (FinnGen)")
...
```{r echo=FALSE}
kable(Mah.jdmD[1:21, .(Rank, Trait, mahal, N1)]) %>% kable_styling()
```
...
fwrite(dpmunc.var, "../data/PAPS_7PC_Var.tsv", sep = "\t")
...
resdpmunc <- readRDS("../data/PAPS_1213_psm_data.rds")
resdpmunc <- data.table(Label = names(resdpmunc$calls$cl), DPMUnc.cl = resdpmunc$calls$cl)

p1213 <- ps2s[ PC %in% paste0("PC", c(1,2,13)), .(Trait, Label, PC, Delta, Trait_long, Trait_class, stars)]
p1213 <- data.table::dcast(p1213, Label + Trait + Trait_class  + Trait_long ~ PC, value.var = c("Delta", "stars"))
p1213 <- merge(p1213, resdpmunc, by=c("Label"), all.x = TRUE)
...
colp <- palette[1:max(p1213$DPMUnc.cl)]

p1213[, DPMUnc.cl:=factor(DPMUnc.cl, levels = 1:10)]


pcbiplot <-  ggplot(p1213, aes(x = Delta_PC1, y = Delta_PC2, colour = DPMUnc.cl, label=Label))+
    ...
    geom_label_repel(size = 4, seed = 15, force = 20, max.overlaps = 20, data = p1213[DPMUnc.cl == "6" | grepl("meta", Label) ], show.legend = FALSE) +
...
We have analysed PAPS projection in the context meta-analyses of 3 IMD of interest and more IMD from our collection. Many projected traits from our collection had very small projections, either because they contain no detectable signal by the features or because they are underpowered (eg. because of low sample size). To focus on traits with relevant signals, we selected IMDs that were significantly different from a null GWAS projection across all features, and then removed redundant projections for better visualisation. 

We applied two clustering methods and one distance method to investigate which IMD are more genetically similar to PAPS. These methods have different advantages and shortcomings, so we don't expect them to fully agree on their results. In in summary, both hierarchical clustering and Mahalanobis distance use full projection information (ie. all PCs), but the hierarchical clustering can get unstable when considering many dimensions and the same time. Mahalanobis offers a more stable metric, but both methods have the shortcoming of not taking uncertainty around projections into account. To address this, we used a novel clustering method, DPMUnc, which clusters projections while accounting for uncertainty. We used projections from three PCs (1, 2, and 13) for which PAPS is significant, and thus we considered relevant.
...
Lastly, JIA, myastenia gravis, RA, and myositis and its subtypes (ie. dermatomyositis, polymyositis, dermatopolymyositis) are supported by both hierarchical clustering and Mahalanobis method as closer to PAPS, but not by DPMUnc. JIA, RA, Felty syndrome, (Early-onset) myastenia gravis, and Myositis and all its subtypes (except for dermatopolymyositis) are grouped together on a different cluster (lighter green cluster) according to DPMUnc. Dermatopolymyositis forms a singleton cluster (darker blue), as its projection is characterised by weak PC1-PC2 signals, but a stronger PC13 signal.
Regarding meta-analyses, both hierarchical clustering and Mahalanobis agree that SLE is the closest to PAPS of the three. DPMUnc does not cluster PAPS with any meta-analysis, and SLE seems to be further apart than the other two meta-analyses. This is likely due to the extreme signal SLE has for PC1 and PC2. Both MS and vasculitis meta-analyses are assigned by DPMUnc to the null cluster. While both meta-analyses have relatively large sample sizes (>3,000 cases), their signal for the selected PAPS PCs is weak.
