...
# These can be used for supplementary tables
...
# sed -i 's/ textLength="[^"]*"//' Myositis_bhattacharyya_heatmap.svg
...
# Trick: If modifying the text (like font, etc.), sometimes the text is fixed within a box, and adding anything will make it 
# squeeze in the box, which is not ideal. TO avoid this behavior, apply the following one-liner below to the file:
# sed -i 's/ textLength="[^"]*"//' sdiag_Bh_DP.svg
# This will remove the fixed textLength from the XML.
...
## CONTINUE HERE
# Check that coloc results haven't changed

coloc <- fread("../tables/coloc_results_dfilt-v2.tsv", na.strings = "")

## The commented step below extracts the list of candidate SNPs and driver SNPs in coloc to map to genes using mapping-genes.py,
## which will create the mapped.genes.tsv
## This bit is not reproducible because I couldn't find the code for bestsnp.rsids.tsv (likely extracted from some summary stats).
## I need to update the code to reflect the proper extraction.
# 
...
cts <- cts[, .(driver.rsid, driver.nearestgene, bestsnp.rsid, bestsnp.novel, pbest.myos, mlabel, fig.label, pairwise_fdr, H4)][order(driver.nearestgene, mlabel, fig.label)]
