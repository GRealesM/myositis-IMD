...
The figure looks a bit small due to html limitations, but you can download it and see it full size.
...
We computed the pairwise FDR for all myositis-IMD pairs and filtered out SNPs in close proximity (< 1Mb apart), resulting in 58 myositis-IMD significant pairs comprising 14 driver SNPs and 11 clusters or independent, non-overlapping regions. None of the 14 SNPs with shared associations are genomewide significant (P < 5e-8) in their individual GWAS (see pdriver.myos in the table below).

```{r echo=FALSE}

coloc <- fread("../tables/coloc_results.tsv", na.strings = "")

# Add some info to the coloc table, and extract rsids to map
best.rsids <- fread("../data/bestsnp.rsids.tsv")
names(best.rsids) <- c("bestsnp", "best.rsid")
...
man <- man[,.(SNPID, pid)]
names(man)[1] <- "driver.rsid"

coloc <- merge(coloc, man)
coloc <- merge(coloc, best.rsids, by="bestsnp")

coloc[H4 > 0.5 & pbest.myos > 5e-8 & pbest.myos > pbest.myos.region, bestsnp.novel:="Yes"] # Add novelty

# # Get rsids to map them to genes
# snp.to.map <- data.table(SNP = unique(c(coloc[H4 > 0.5, unique(driver.rsid)], coloc[H4 > 0.5, unique(best.rsid)])))
# fwrite(snp.to.map, "../data/snp.to.map.tsv", sep="\t")


# 
# rsids.dr <- coloc[, .(unique(driver.rsid))]
# fwrite(rsids.dr, "../data/rsids.dr.tsv", col.names = F)
# 
# ## Now we'll retrieve the mapped genes for the driver and candidate SNPs

# library(GenomicRanges)
# library(TxDb.Hsapiens.UCSC.hg38.knownGene)

drs <- unique(coloc[, .(pid, driver.rsid)])
drs[, c("chr", "bp"):=tstrsplit(pid, ":")][, c("chr", "bp"):=lapply(list(chr,bp), as.numeric)][, chr:=paste0("chr",chr)]


snp_ranges <- GRanges(seqnames = drs$chr,
                      ranges = IRanges(start = drs$bp,
                                       end = drs$bp),
                      names = drs$driver.rsid)
txdb <- TxDb.Hsapiens.UCSC.hg38.knownGene
txdb <- genes(txdb)
nearest_genes <- GenomicRanges::nearest( snp_ranges, txdb,select="all")
annres <- data.table(rsid = snp_ranges[nearest_genes@from]$names, gene_id = txdb[nearest_genes@to]$gene_id)


mart <- useMart("ENSEMBL_MART_ENSEMBL")
mart <- useDataset("hsapiens_gene_ensembl", mart)
tt <- getBM(attributes = c("entrezgene_id", "external_gene_name", "hgnc_symbol"), filters = c("entrezgene_id"), values = annres$gene_id, mart = mart)
tt %<>% as.data.table
tt[, entrezgene_id:=as.character(entrezgene_id)]
annres <- merge(annres, tt, by.x = "gene_id", by.y = "entrezgene_id", all.x=TRUE)
annres[external_gene_name == "", external_gene_name:=NA]
...
coloc <- merge(coloc, man)
coloc[H4 > 0.5 & pbest.myos > 5e-8 & pbest.myos > pbest.myos.region, bestsnp.novel:="Yes"] # Add novelty
coloc[ H4 > 0.5, .(pid, driver.rsid, trait.myos, trait.other, H4)][order(pid)] %>% kable %>% kable_styling(full_width = FALSE)
...
To explore a wider range of driver SNPs across myositis-IMD pairs, we relaxed the FDR threshold to 0.5 (ie. more likely to be significant for both diseases than not), and we obtained 80 driver SNPs and 482 myositis-IMD pairs. We applied coloc to each myositis-IMD pair regions surrounding each driver SNP associated to the pair, using a 1Mb window around and assuming a single causal variant. 

This resulted in 51 myositis-IMD pairs and 14 driver SNPs in 12 independent regions with $P(H4) >0.5$ (ie. highest probability for a shared variant for both IMD in the region). 

As mentioned earlier, the driver SNP might not be the causal one, but be in LD with the causal variant instead. Thus, we also report SNP with the highest posterior probability of being the shared causal variant in each region. We'll call **novel associations** those SNPs that (1) have high posterior to be shared (H4 > 0.5) between a given myositis and an IMD, (2) are not genome-wide significant (ie. P > 5e-8) in the myositis GWAS, and (3) are not the top hit (ie. the one with the lowest P-value) in the 1Mb region defined for coloc.
...
Note that we here we show associations with strong association probability (H4 > 0.8), but below we look at each region in more detail.

We found 24 high confidence associations and 9 associated candidate SNPs (8 of them novel for myositis and its subtypes) across 5 independent genomic regions. These associations included polymyositis with hyperthyroidism, JIA, MPO+AAV, and myastenia gravis, and pooled myositis with hypo- and hyperthyroidism, JIA, MPO+AAV, myastenia gravis, PBC, Sjögren's syndrome and systemic sclerosis. 

```{r echo=FALSE}

mapped.genes <- "1:113834946, PTPN22, rs2476601,
1:113761186, PHTF1, rs6679677,
2:100143015, AFF3, rs11692867,
2:191038032, STAT1, rs3821236,
2:191071078, STAT1, rs10168266,
3:28033394, EOMES and CMC1, rs451375,
3:28033679, EOMES and CMC1, rs819991,
4:122629959, IL21, rs13132308,
6:166993145, CEP43, rs415890,
7:128936032, IRF5, rs3757387,
7:129044262, TNPO3, rs62478615,
7:129011468, TNPO3, rs36073657,
7:129061277, TNPO3, rs13246321,
7:129023119, TNPO3, rs13236009,
7:37352216, ELMO1, rs2700984,
8:11480078, BLK, rs2618476,
8:11486464, BLK, rs2736340,
11:64340263, CCDC88B, rs663743,
11:64369120, RPS6KA4, rs1662188,
12:111535554, ATXN2, rs597808,
12:111472415, ATXN2, rs10774625,
17:39907128, GSDMB, rs883770,
17:75215547, NUP85, rs9909306"

split_text <- unlist(strsplit(mapped.genes, ", |,\n"))  # Split text based on comma and space
column1 <- split_text[c(TRUE, FALSE, FALSE)]  # Select the odd-indexed elements for column 1
column2 <- split_text[c(FALSE, TRUE, FALSE)] # Select the even-indexed elements for column 2
column3 <- split_text[c(FALSE, FALSE, TRUE)] # Select the even-indexed elements for column 3
mapped.genes <- data.table(bestsnp = column1, mapped.genes = column2, bestsnp.rsid = column3)

# We need to ensure that we assign the genes to the pids as well
mg <- merge(coloc[, .(pid, bestsnp)], mapped.genes, by="bestsnp")
mg <- unique(mg[, .(pid, mapped.genes)])

sumcoloc <- merge(coloc, mg, by="pid", all.x = TRUE, allow.cartesian = TRUE)
sumcoloc <- merge(sumcoloc, mapped.genes[, .(bestsnp, bestsnp.rsid)], by="bestsnp", all.x = TRUE, allow.cartesian = TRUE) # Add bestsnp.rsids
sumcoloc[, clabel:=paste( mapped.genes, driver.rsid, sep=" / ")]


sumcoloc[ H4 > 0.8, .(trait.myos, trait.other, pid, H4, bestsnp, bestsnp.pp, mapped.genes, bestsnp.novel)][order(trait.myos, trait.other, bestsnp)] %>% kable(digits = 32) %>% kable_styling

```


Most of these high-confidence candidate SNPs come from pooled myositis, which are meta-analyses combining all the myositis other subtypes combined. Two candidate SNPs (1:113834946, and 1:113761186) appear in polymyositis and myositis for the same associations (hypo/hyperthyroidism and myastenia gravis, and jia, respectively). It might be the case that some of these associations observed in pooled myositis come from other subtypes with smaller H4 on their own. Let's check if that's the case.



```{r echo=FALSE}
# which myositis SNPs are explained by other subtypes?
snps.myo <- sumcoloc[ H4 > 0.8 & trait.myos %in% c("myo.r", "myo.m"), unique(bestsnp)]
snps.pm <- sumcoloc[ H4 > 0.8 & trait.myos %in% c("pm.r", "pm.m"), unique(bestsnp)]
snps.myo.and.pm <- intersect(snps.myo, snps.pm)
#snps.myo.no.pm <- setdiff(snps.myo, snps.pm)
   
sumcoloc[bestsnp %in% snps.myo & (trait.myos %in% c("myo.r", "myo.m") | H4 < 0.8) & H4 > 0.5, .(trait.myos, trait.other, pid, H4, bestsnp, bestsnp.pp, mapped.genes, bestsnp.novel) ][order(bestsnp)]
...
keypids <- sumcoloc[ H4 > 0.5, unique(pid)]

sumc2 <- sumcoloc[pid %in% keypids, .(clabel, trait.myos, trait.other, H4)]
sumc2 <- sumc2 %>% tidyr::complete(clabel, trait.myos, trait.other ) %>% as.data.table()
sumc2[, clabel:=factor(clabel, levels = rev(unique(clabel)))]
sumc2[, trait.other:=factor(trait.other, levels = rev(unique(trait.other)))]
sumc2[,flag:=ifelse(H4>.8, "high", ifelse(H4>.5, "med", "low"))]

# 
# c2[, clabel:=factor(clabel, levels=snps$pid)]
# 
# ## flag > .8, > .5
# sumc2[,flag:=ifelse(H4>.8, "high", ifelse(H4>.5, "med", "low"))]
# sumc2=sumc2[order(H4)]
# 
# ggplot() +
#     geom_tile(aes(x =  trait.other, y = clabel),
#               lwd=.5, linetype = 1, data=sumc2[], fill="white", colour="black") +
#     geom_tile(aes(x =  trait.other, y = clabel, colour = flag, fill = H4),
#               lwd=1, linetype = 1, data=sumc2[H4>.5]) +
#     scale_colour_manual(values=c(high="yellow",med="green",low="grey"), guide=guide_none()) +
#     scale_fill_gradient(na.value = "white") + #, low="grey90", high="#56B1F7")+
...
# 
# sumc3 <- sumcoloc[H4 > 0.5, .(clabel, trait.myos, trait.other, H3,H4)]
# sumc3 <- sumc3 %>% tidyr::complete(clabel, trait.myos, trait.other ) %>% as.data.table()
# sumc3[, clabel:=factor(clabel, levels = rev(unique(clabel)))]
# sumc3[, trait.other:=factor(trait.other, levels = rev(unique(trait.other)))]
# 
# ggplot(sumc3, aes(x =  trait.other, y = clabel, fill = H3)) +
...
```{r echo=FALSE}
coloc[ pid == "1:113834946" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H3, H4, bestsnp, bestsnp.pp, pbest.myos, bestsnp.novel)] %>% kable(digits = 32) %>% kable_styling
```
...
[1:113834946](https://genetics.opentargets.org/variant/1_113834946_A_G) corresponds to rs2476601 an important missense variant located in the **PTPN22** gene, associated with numerous IMD, such as RA, type 1 diabetes, SLE, and others.
...
None of these SNPs are genomewide significant for either Myositis (Rothwell) or Polymyositis (Rothwell or Miller). 1:113834946 is a novel association for pooled myositis, and 1:113761186 is a novel association for polymyositis.


In the plot below, we show the region for all IMD involved. Note that some of them did have pairwise_fdr < 0.5 but not H4 > 0.5, so we won't discuss them.
...
```{r echo=FALSE}
coloc[ pid == "2:191071078" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H3, H4, bestsnp, bestsnp.pp, pbest.myos, bestsnp.novel)] %>% kable(digits = 32) %>% kable_styling
```


![2:191071078 region Manhattan plot. The driver SNP is signalled by a red line.](../figures/coloc_chr2_2.png)

We find two H4 > 0.5 associations in this region between Myositis (Rothwell), hyperthyroidism and systemic sclerosis. While H4 is borderline (H4 = 0.505) for hyperthyroidism, it is very high (H4 > 0.99) for systemic sclerosis. In the latter case, the SNP with the highest posterior probability to be the shared SNP is [2:191038032](https://genetics.opentargets.org/variant/2_191038032_G_A), rs3821236, an intron variant of **STAT1** not genomewide significant for myositis (Rothwell) but associated with other IMD, such as SLE and RA. This is also a novel association.
...
```{r echo=FALSE}
coloc[ pid == "3:28029953" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H3, H4, bestsnp, bestsnp.pp, pbest.myos, bestsnp.novel)] %>% kable(digits = 32) %>% kable_styling
```
...
```{r echo=FALSE}
coloc[ pid =="4:122194347" & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H3, H4, bestsnp, bestsnp.pp, pbest.myos, bestsnp.novel)] %>% kable(digits = 32) %>% kable_styling
```
...
![12:112468611 region Manhattan plot. The driver SNP is signalled by a red line.](../figures/coloc_chr12.png)
