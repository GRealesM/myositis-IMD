...
# There was an extra DMPMYO by FinnGen, we'll remove it. We'll also remove myositis from FinnGen, as it contains unclassified myositis and it showed to be very different from the other myositis datasets
# Remove PAPS as well
tr <- c(tr, "DERMATOPOLY_FG_FinnGen_FinnGenR7_1", "M13_MYOSITIS_FinnGen_FinnGenR7_1")
...
# We keep the initial matrices for make_bold_names(), since I don't have the time to adapt it now
bhatta.d1 <-  acast(bhd, T1~T2, value.var = "bhat.dist")
bh1 <- bhatta.d1 %>% as.dist %>% hclust
bhatta.d2 <-  acast(bhd, T1~T2, value.var = "logbhat") 
bh2 <- bhatta.d2 %>% as.dist %>% hclust

bh1.ph <- pheatmap(bhatta.d1,
                cluster_cols = bh1, cluster_rows = bh1,
                fontsize = 9, color = mahalcolp, show_colnames = FALSE, labels_row = make_bold_names(bhatta.d1, rownames, papsb))
bh2.ph <- pheatmap(bhatta.d2, 
                cluster_cols = bh2, cluster_rows = bh2,
                fontsize = 9, color = mahalcolp, show_colnames = FALSE, labels_row = make_bold_names(bhatta.d2, rownames, papsb))

bh3.ph <- pheatmap(bhatta.d2, 
                cluster_cols = bh1, cluster_rows = bh1,
                fontsize = 9, color = mahalcolp, show_colnames = FALSE, labels_row = make_bold_names(bhatta.d2, rownames, papsb))
