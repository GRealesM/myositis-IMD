...
coloc[ pairwise_fdr < 0.05, .(pid, trait.myos, trait.other, pairwise_fdr, pdriver.myos)][order(pid)] %>% kable %>% kable_styling(full_width = FALSE)
```
...
column3 <- split_text[c(FALSE, FALSE, TRUE)] # Select the even-indexed elements for column 2
mapped.genes <- data.table(bestsnp = column1, mapped.genes = column2, bestsnp.rsid = column3)

sumcoloc <- merge(coloc[!is.na(bestsnp)], mapped.genes, by="bestsnp", all.x = TRUE)
sumcoloc[, clabel:=paste(bestsnp.rsid, mapped.genes, sep=" / ")][ ]
...
**PLOT IDEA** A myositis vs candidate SNP heatmap or grid, with squares representing H4, and scaled by colour. That would be an easy way to see the relationships between candidate SNPs.
Another idea could involve myositis-IMD-disease. Maybe improve the table above to identify pooled and subtypes associations and cluster them by disease?
...
sumc2 <- sumcoloc[H4 > 0.5, .(clabel, trait.myos, trait.other, H4)]
...
ggplot(sumc2, aes(x =  trait.other, y = clabel, fill = H4)) +
...
### Chromosome 7.1 region (7:128933913, 7:128954129, 7:128977412)


```{r echo=FALSE}
coloc[ pid %in% c("7:128933913", "7:128954129", "7:128977412") & (H4 > 0.5), .(pid, trait.myos, trait.other, pairwise_fdr, H3, H4, bestsnp, bestsnp.pp, pbest.myos, bestsnp.novel)][order(trait.myos)] %>% kable(digits = 32) %>% kable_styling
```
...
![7.1 (7:128933913, 7:128954129, 7:128977412) region Manhattan plot. The driver SNP is signalled by a red line.](../figures/coloc_chr7_1.png)
