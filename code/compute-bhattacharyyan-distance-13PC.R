...
bh.list <- lapply(seq_along(qs$Label), function(i){
                ...
                rr1  <- ps[ Label == qs$Label[i], Delta]
                names(rr1) <- ps[ Label == qs$Label[i], PC]
                ...
                bhd <- sapply(seq_along(qs$Label), function(x){
                        rr2 <- ps[ Label == qs$Label[x], Delta]
                        names(rr2) <- ps[ Label == qs$Label[x], PC] 
...
bh.list
fwrite(bh.list, "../data/bh_dist_13PC.tsv", sep="\t")
