...
length(unique(resgp$term_id)) # 667
...
pc.summary <- data.table(
                ...
                excl2.pathways = resgp[ ncopies <= 2, .N, by=PC ]$N,
                genes.excl2   = resgp[ ncopies <= 2, intersection_genes %>% strsplit(split = ", ") %>% unlist() %>% unique() %>% length(), by=PC]$V1
)
pc.summary
...
fwrite(resgp[ncopies <=2], "../tables/SupplTable_exclusive_pathways.tsv", sep="\t")
```
